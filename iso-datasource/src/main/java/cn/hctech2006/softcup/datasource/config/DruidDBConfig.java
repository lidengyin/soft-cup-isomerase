package cn.hctech2006.softcup.datasource.config;


import cn.hctech2006.softcup.datasource.dynamic.NewDynamicDataSource;
import cn.hctech2006.softcup.datasource.util.PropertiesUtil;
...
public class DruidDBConfig {
    ...
    private String name = PropertiesUtil.getProperty("spring.datasource.one.druid.name");
    @Bean
    @Primary
    ...
    public NewDynamicDataSource dynamicDataSource(){
        NewDynamicDataSource dynamicDataSource = new NewDynamicDataSource();
        dynamicDataSource.setDebug(false);
        String datasourceId = DigestUtils.md5DigestAsHex("adiDataSource".getBytes());
        logger.info("--------Druid数据源设置默认数据源为主数据源:"+datasourceId+"------------------");
        dynamicDataSource.setDefaultTargetDataSource(dataSource());
        Map<Object, Object> targetDataSources = new HashMap<>();

        targetDataSources.put(datasourceId, dataSource());
        logger.info("--------Druid数据源设置缓存数据源库------------------");
